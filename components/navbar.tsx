...
export default function Navbar({ hideThemeToggle = false }: NavbarProps) {
  ...
  return (
    <>
      ...
      {/* Mobile menu overlay - Fora do header para garantir z-index correto */}
      {mobileMenuOpen && (
        <div 
      ...
      {/* Mobile menu - Fora do header para garantir z-index correto */}
      <div
        className={cn(
          "lg:hidden fixed inset-y-0 right-0 z-[9999] w-72 max-w-[85vw] bg-white dark:bg-slate-900 shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col",
      ...
      >
        <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
          <Link href="/" className="flex items-center" onClick={() => setMobileMenuOpen(false)}>
            <span className="sr-only">Alexandre Guimarães</span>
            <div className="h-7 w-7 rounded-full overflow-hidden mr-2 flex-shrink-0">
              <img
                src="/images/guimas-palestrante-header.png"
                alt="Alexandre Guimarães"
                width={28}
          ...
          <button
            type="button"
            className="rounded-md p-1 text-gray-900 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 flex-shrink-0"
            onClick={() => setMobileMenuOpen(false)}
